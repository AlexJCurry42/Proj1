...
export function initAboutModal() {
  ...
  document.getElementById('about-content').innerHTML = `
    ...
    <p class="hint">Want the sharpest possible view of a region? Slide the spectrum rail to <strong>Pan-STARRS</strong> — a modern CCD survey, far deeper and cleaner than the classic photographic DSS2 (it covers the sky north of declination −30°). Every stop on the rail is real observational data, so image quality is set by each survey's telescope and era, not by your screen. The <strong>Sharpen imagery</strong> checkbox (Display) adds a strong two-scale optical unsharp mask — a fine pass that tightens star profiles and a wider "clarity" pass that lifts nebular structure. Pure local contrast, never invented detail — at some cost in battery and, on older phones, smoothness; it also amplifies film grain and JPEG noise, which is honest but not always pretty.</p>
    <h3>Catalogs</h3>
    <ul>
